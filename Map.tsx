...
const Map: React.FC<MapProps> = ({ 
  ...
  useEffect(() => {
    ...
    const cellSize = selectedFire ? 0.005 : 0.08;

    // Helper for rendering grid
    const renderGrid = (dataSamples: any[], getStyles: (val: number) => { color: string, opacity: number }) => {
  ...
  useEffect(() => {
    ...
    if (geojson) {
      geojsonLayerRef.current = L.geoJSON(geojson, { 
        style: { color: '#ffffff', weight: 2, fillOpacity: 0.05, dashArray: '8, 8' } 
